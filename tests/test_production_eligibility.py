...
# The decision fixture is valid for 2026-08-22T12:00:00Z .. 13:00:00Z. Verification time is
# pinned rather than taken from the clock, so these tests keep asserting what they were
# written to assert instead of expiring into failure the hour after they were recorded.
WITHIN_WINDOW = datetime(2026, 8, 22, 12, 30, tzinfo=timezone.utc)
AFTER_WINDOW = datetime(2026, 8, 22, 13, 0, tzinfo=timezone.utc)
...
class DecisionExpiryTest(unittest.TestCase):
    ...
    def test_expired_decision_is_refused(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            payload = Path(directory) / "payload.bin"
            signature = Path(directory) / "signature.bin"
            with self.assertRaisesRegex(ValueError, "expired at"):
                ELIGIBILITY.verify_response(
                    response,
                    bundle,
                    policy,
    ...
    def test_expiry_boundary_is_exclusive(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            payload = Path(directory) / "payload.bin"
            signature = Path(directory) / "signature.bin"
            just_inside = AFTER_WINDOW - timedelta(microseconds=1)
            self.assertEqual(
                ELIGIBILITY.verify_response(
                    response,
                    bundle,
