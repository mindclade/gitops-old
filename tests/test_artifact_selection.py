...
VALIDATE_SELECTIONS = ROOT / "scripts/validate-deployment-selections.py"
...
class ArtifactSelectionTest(unittest.TestCase):
    def test_quarantine_v2_envelopes_are_empty_and_valid(self) -> None:
        result = subprocess.run(
            [sys.executable, str(VALIDATE_SELECTIONS)],
            capture_output=True,
            check=False,
            text=True,
        )
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn("development=0, staging=0, production=0", result.stdout)
