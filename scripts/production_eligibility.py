...
def verify_response(
    ...
    *,
    ...
    # The three checks above establish that the window is INTERNALLY coherent -- ordered, no
    # wider than six hours, and inside the policy's own validity. None of them reads a clock, so
    # a decision whose window closed weeks ago satisfies all three. Replaying such a decision
    # into `decide` or `publish-decision` -- both of which reach this same function -- had it
    # accepted as a currently-valid eligible result, which makes expires_at decorative at the
    # only point that enforces it.
    #
    # Compared against `now` rather than a freshness bound on evaluated_at: expires_at is the
    # field the issuer controls to say when the decision stops being true, so honouring it is
    # the narrower and more faithful check.
    if now >= expires:
        fail(
            "production eligibility decision expired at "
            f"{expires.isoformat()} (now {now.isoformat()})"
        )
