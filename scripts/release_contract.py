...
def load_policy(path: Path) -> dict[str, Any]:
    ...
    if set(value) != expected:
        raise ValueError(
            f"release handoff policy fields must be exactly {sorted(expected)}"
        )
    ...
    if value.get("evidence_retention") != {
        ...
        raise ValueError(
            "release evidence retention must be P1Y nonproduction and P7Y production"
        )
    exception = value.get("vulnerability_exception")
    if exception != {"approved_by": "@mindclade/security", "maximum_duration_days": 90}:
        raise ValueError(
            "release vulnerability exception policy is not the governed 90-day policy"
        )
    return value
...
def validate_vulnerability(
    ...
    *,
    ...
    if not re.fullmatch(
        r"[0-9]+\.[0-9]+\.[0-9]+", str(value.get("scanner_version", ""))
    ):
    ...
    scanned_at = aware_timestamp(
        value.get("scanned_at"), "vulnerability.scanned_at", errors
    )
    ...
    if not isinstance(counts, dict) or set(counts) != severities:
        errors.append(
            f"vulnerability finding_counts must contain exactly {sorted(severities)}"
        )
        counts = {}
    for severity in severities:
        ...
        if not isinstance(count, int) or isinstance(count, bool) or count < 0:
            errors.append(
                f"vulnerability finding_counts.{severity} must be a non-negative integer"
            )
    ...
    if result == "pass":
        ...
        if critical + high == 0:
            errors.append(
                "a vulnerability exception requires at least one critical or high finding"
            )
        if not isinstance(exception, dict):
            errors.append(
                "approved vulnerability evidence requires one exception object"
            )
        else:
            ...
            if exception.get("approved_by") != exception_policy["approved_by"]:
                errors.append(
                    "vulnerability exception requires @mindclade/security approval"
                )
            ...
            if consumer and exception.get("subject_digest") != subject_digest:
                errors.append(
                    "vulnerability exception must bind the exact release subject digest"
                )
            approved = aware_timestamp(
                exception.get("approved_at"),
                "vulnerability.exception.approved_at",
                errors,
            )
            expires = aware_timestamp(
                exception.get("expires_at"),
                "vulnerability.exception.expires_at",
                errors,
            )
            ...
            if (
                approved
                and expires
                and (expires <= approved or expires - approved > maximum)
            ):
                errors.append("vulnerability exception must expire within 90 days")
            if approved and created_at and approved > created_at:
                errors.append(
                    "vulnerability exception may not be approved after created_at"
                )
            if expires and created_at and expires <= created_at:
                errors.append(
                    "vulnerability exception must be active when the release is created"
                )
    else:
...
def validate_producer_evidence(value: object, policy: dict[str, Any]) -> list[str]:
    ...
    if set(value) != PRODUCER_REQUIRED:
        errors.append(
            f"producer release evidence fields must be exactly {sorted(PRODUCER_REQUIRED)}"
        )
    ...
    for name, image in images.items():
        if not IDENTIFIER.fullmatch(str(name)) or not DIGEST_IMAGE.fullmatch(
            str(image)
        ):
    ...
    for index, artifact in enumerate(artifacts):
        ...
        if set(artifact) != {"name", "type", "uri", "digest", "media_type"}:
            errors.append(
                f"producer artifacts[{index}] has unsupported or missing fields"
            )
    ...
    for artifact_type in sorted(REQUIRED_ARTIFACT_TYPES):
        if len(artifact_types.get(artifact_type, [])) != 1:
            errors.append(
                f"producer must contain exactly one {artifact_type!r} artifact"
            )
    ...
    for index, edge in enumerate(graph):
        ...
        if edge.get("subject_digest") != subject_digest:
            errors.append(
                f"producer evidence predicate {predicate!r} does not bind the subject"
            )
        artifact = artifact_by_name.get(str(edge.get("artifact", "")))
        if artifact is None or artifact.get("type") != PREDICATE_ARTIFACT_TYPES.get(
            predicate
        ):
            errors.append(
                f"producer evidence predicate {predicate!r} references the wrong artifact"
            )
        expected_result = (
            "approved"
        ...
        if edge.get("result") != expected_result:
            errors.append(
                f"producer evidence predicate {predicate!r} must be {expected_result}"
            )
    if predicates != set(PREDICATE_ARTIFACT_TYPES):
        errors.append(
            "producer evidence graph must cover each required predicate exactly once"
        )
    ...
    if not isinstance(attestations, dict) or set(attestations) != {
        ...
        errors.append(
            "producer attestations must contain exactly build and qualification"
        )
    else:
        ...
        if len(roots) != 2:
            errors.append(
                "producer build and qualification attestor roots must be distinct"
            )
    ...
    migration = (
        value.get("migration") if isinstance(value.get("migration"), dict) else {}
    )
    ...
    if migration.get("required") is True:
        if migration_artifact is None or migration_artifact.get("type") != "migration":
            errors.append(
                "producer required migration must reference one migration artifact"
            )
    elif migration.get("artifact") is not None:
        errors.append(
            "producer migration artifact must be null when migration is not required"
        )
    ...
    if rollback.get("strategy") == "previous-release":
        if not nonempty(previous_release) or not SHA256.fullmatch(str(previous_digest)):
            errors.append(
                "producer previous-release rollback requires exact prior lineage"
            )
...
def project_release_evidence(
    ...
    *,
    ...
    if not PROJECT.fullmatch(deployment_project) or not IDENTIFIER.fullmatch(
        deployment_attestor
    ):
        raise ValueError(
            "deployment attestor project and name must be stable identifiers"
        )
...
def projection_errors(
    ...
    if producer_ref.get("digest") != actual_digest:
        errors.append(
            "producer_evidence.digest does not match canonical producer bytes"
        )
    try:
...
def active_exception_errors(record: dict[str, Any], as_of: dt.datetime) -> list[str]:
    ...
    if (
        ...
        or vulnerability.get("result") != "approved-exception"
    ):
        return []
