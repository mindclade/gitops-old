...
def load_selection(environment: str, errors: list[str]) -> dict[str, dict]:
    ...
    if (
        api_version not in {"mindclade.dev/v1", "mindclade.dev/v2"}
    ...
    if not isinstance(raw_apps, list):
        ...
        return {}
    if api_version == "mindclade.dev/v2" and raw_apps:
        errors.append(
            f"{path.relative_to(ROOT)}: the v2 compatibility envelope must remain empty "
            "until the coordinated release-contract migration"
        )
        return {}
